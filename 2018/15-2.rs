...
fn main () {
    ...
    let mut grid_ : Vec<Vec<char>> = Vec::new();
    let mut elves_ : BTreeMap<(usize,usize),i16> = BTreeMap::new();
    let mut goblins_ : BTreeMap<(usize,usize),i16> = BTreeMap::new();
    let mut allunits_ : BTreeSet<(usize,usize)> = BTreeSet::new();
    ...
    for line in input {
        ...
        for ch in line.chars() {
            one_line.push(ch);
            if ch == 'E' {
                elves_.insert((y,x),200);
                allunits_.insert((y,x));
            } else if ch == 'G' {
                goblins_.insert((y,x),200);
                allunits_.insert((y,x));
            }
            x += 1;
        }
    ...
    loop {
        ...
        rounds = 0;
        let mut any_elves_dead = false;

        let mut grid = grid_.clone();
        let mut elves = elves_.clone();
        let mut goblins = goblins_.clone();
        let mut allunits = allunits_.clone();
    ...
    let mut hp_s : u64 = 0; 
    if elves_.len() > 0 {
        for hp in elves_.values() {
            
            hp_s += *hp as u64;
        }
    }
    println!("damage {}, rounds: {} x {} = {}", damage, rounds, hp_s, rounds * hp_s);
