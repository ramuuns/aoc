    ...
    (v,t) => { 
        ...
        console.log(v.split("\n").filter(v => v!="").map(line => { 
            ...
            if (matches[4]) {
                ...
                variants.forEach(v => { 
                    if (v[0] == "i") { 
                        immunity.push(v.split("to ")[1]);
                    } else { 
                        v.split("to ")[1].split(", ").forEach( w => weakness.push(w)); 
                    } 
                }); 
            }
            idx++;
            var im = immunity.length ? 'vec!["'+immunity.join('","')+'"]' : "Vec::new()";
            var wk = weakness.length ? 'vec!["'+weakness.join('","')+'"]' : "Vec::new()";
...
*/
    ...
    loop {
        if armies_by_type[0].len() == 0 || armies_by_type[1].len() == 0 {
            break;
        }
        let mut targets_to_select_by_type = armies_by_type.clone();
        let mut target_from_to = HashMap::new();

        let mut army_tgt_selection_ord : BTreeMap<(i64,i8),u8> = BTreeMap::new();
        for (_k,army) in &armies {
        ...
        for (_k, army_id) in army_tgt_selection_ord {
            let me = armies.get(&army_id).unwrap();
            ...
            if targets_to_select_by_type[other_type].len() > 0 {
                ...
                if best.0 != FAKE_ID {
                    println!("army {} selected target {}", army_id, best.0);
        ...
        for (_k,army_id) in army_attack_ord {
            if armies.contains_key(&army_id) && target_from_to.contains_key(&army_id) {
                print!("army {} attacks {} ", army_id, target_from_to[&army_id]);
                let mut target = armies[&target_from_to[&army_id]].clone();
                let me = armies[&army_id].clone();
                let old_units = target.units;
                if !target.defend(&me) {
                    println!("killing remaining {} units", old_units);
                    armies.remove(&target.id);
                    armies_by_type[target.atype].remove(&target.id);
                } else {
                    println!("killing {} units" ,old_units - target.units);
                    armies.insert(target.id, target);
                }
            }
        }
    }
