...
fn main () {
    ...
    for event in input {
        if event.event_type == EV_GUARD_CHANGE {
            ...
            if current_guard.total_sleep > max_sleep {
                max_sleep = current_guard.total_sleep;
                max_sleep_guard_id = current_guard_id;
            }
        }
    }
    let current_guard = guards.entry(max_sleep_guard_id).or_insert(Guard::new());
    let mut max_minute = 0;
    let mut max_per_minute = 0;
    for i in 0..60 {
        if current_guard.sleep_times[i] > max_per_minute {
            max_per_minute = current_guard.sleep_times[i];
            max_minute = i as u16;
        }
    }
    println!("The result: {}", max_minute * max_sleep_guard_id);
}
