...
fn main () {
    ...
    let mut stack : Vec<(u16,u16,Vec<u16>)> = Vec::new();
    let mut state = STATE_READ_HEADER_CHILDREN;
    let mut children : u16 = 0;
    let mut meta : u16 = 0;
    let mut sum : u16 = 0;
    let mut childvalues : Vec<u16> = Vec::new();
    ...
    for i in data {
        ...
        if state == STATE_READ_HEADER_CHILDREN {
            ...
            if children > 0 {
                ...
                stack.push((children,meta,childvalues.clone()));
                childvalues = Vec::new();
            ...
            //state == STATE_READ_DATA
            if childvalues.len() > 0 {
                match childvalues.get(i as usize - 1 ) {
                    Some(n) => { 
                        sum += n;
                      //  println!("Adding: {}",n);
                    },
            ...
            while meta == 0 {
                if stack.len() > 0 {
                    let (popchildren,popmeta,popcv) = stack.pop().unwrap();
                    meta = popmeta;
                    children = popchildren;
                    children -= 1;
                    childvalues = popcv;
                    childvalues.push(sum);
