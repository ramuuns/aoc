...
56 93 4"
  ...
  def parse_maps([nrs | rest], [{from,to, list} | res], :parse_numbers) do
    [dest, src, range] = nrs |> String.split(~r"\s+", trim: true) |> Enum.map(&String.to_integer/1)
  ...
  def find_location(num, [{_,_, mappings} | rest]), do: find_location(map_number(num, mappings), rest)

  def map_number(num, []), do: num
  def map_number(num, [{src, dest, range} | _]) when num >= src and num < src + range, do: num - src + dest
  def map_number(num, [_|rest]), do: map_number(num, rest)

  def find_min_location_ranges(seeds, maps) do
    seeds |> Enum.reduce([], fn 
  ...
  def range_to_next_ranges({start, range}, mappings) do
    st = map_number(start, mappings)
    r_end = map_number(start+range, mappings)
    if r_end - st == range do
      # no split needed
      [{st, range}] 
    else
  ...
  def range_to_ranges([item | src_ranges], [], mappings, res), do: range_to_ranges(src_ranges, mappings, mappings, [ item | res ])
  def range_to_ranges([{start, range} | src_ranges], [{src, dst, sd_range} | rest], mappings, res) do
    cond do
      start >= src and start < src + sd_range and  start + range < src + sd_range ->
        range_to_ranges(src_ranges, mappings, mappings, [ { start - src + dst, range} | res])
      start >= src and start < src + sd_range ->
        st = start - src + dst
        rng = (src + sd_range) - start
        range_to_ranges([{ start + rng, range - rng } | src_ranges], mappings, mappings, [ {st, rng} | res ])
      src > start and start + range > src and start + range > src + sd_range ->
        range_to_ranges([{ start, src - start }, { src + sd_range, start + range - (src + sd_range) } | src_ranges ],
          mappings, mappings, [{ dst, sd_range } | res ])
      src > start and start + range > src ->
        range_to_ranges([{ start, src - start} | src_ranges], mappings, mappings, [{dst, range - (src - start)} | res])
      true -> range_to_ranges([{start, range} | src_ranges], rest, mappings, res)
    end 
  end
