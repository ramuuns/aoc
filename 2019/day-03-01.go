...
func main() {
	...
	for _, segment := range wire1 {
		...
		switch dir {
		case "U":
			line := VLine{x: x, start: y, end: y + len_as_int}
			vlines = append(vlines, line)
			y += len_as_int
		case "D":
			line := VLine{x: x, start: y - len_as_int, end: y}
			vlines = append(vlines, line)
			y -= len_as_int
		case "R":
			line := HLine{y: y, start: x, end: x + len_as_int}
			hlines = append(hlines, line)
			x += len_as_int
		case "L":
			line := HLine{y: y, start: x - len_as_int, end: x}
			hlines = append(hlines, line)
			x -= len_as_int
		}
	}
