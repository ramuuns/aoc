...
func main() {
	...
	for {
		...
		for ; y < 100; y++ {
			pr := p.duplicate()
			ch, _ := pr.run_until_output(Point{start.x, start.y - y})
			if ch == 0 {
				success = false
				break
			}
		...
		if success {
			...
			for ; x < 100; x++ {
				pr := p.duplicate()
				ch, _ := pr.run_until_output(Point{start.x + x, start.y - y})
				if ch == 0 {
					success = false
					break
				}
		...
		if success {
			fmt.Println(start.x*10000 + start.y - y)
			//\0/ print results and exit
			break
		}
		start.y += 1
		for {
			start.x += 1
			pr := p.duplicate()
			ch, cont := pr.run_until_output(start)
			if ch == 1 {
				break
			}
			if !cont {
